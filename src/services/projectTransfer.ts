...
function normalizeChapter(raw: unknown, index: number): Chapter {
  ...
  return {
    ...
    // API 失败降级的模板稿标记：丢失会把非正式稿误当定稿自动锁章
    conservativeDraft: typeof r.conservativeDraft === 'boolean' ? r.conservativeDraft : undefined,
    // 本章写后事实快照（账本连续性）：丢失导致后续章记忆检索断档
    factSnapshot: isRecord(r.factSnapshot)
      ? (r.factSnapshot as unknown as Chapter['factSnapshot'])
      : undefined,
