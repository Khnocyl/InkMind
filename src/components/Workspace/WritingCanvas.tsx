...
export const WritingCanvas: React.FC<WritingCanvasProps> = ({
  ...
  const [notesOpen, setNotesOpen] = useState(() => !!(chapter.authorNotes?.trim()));
  const openTodoCount = (chapter.revisionTodos || []).filter((t) => t.status === 'open').length;
  const [todosOpen, setTodosOpen] = useState(
    () => openTodoCount > 0 || (chapter.revisionTodos || []).length > 0
  );
  ...
  // 换章时：有批注/待修则自动展开
  useEffect(() => {
    setNotesOpen(!!(chapter.authorNotes?.trim()));
    const todos = chapter.revisionTodos || [];
    setTodosOpen(todos.some((t) => t.status === 'open') || todos.length > 0);
    setTodoDraft('');
    setHighlightedTodoId(null);
    setWavyRanges([]);
  }, [chapter.id]);
  ...
  return (
    <main className="flex-1 bg-white text-black overflow-y-auto p-8 lg:p-12 relative flex flex-col items-center select-none theme-paper">
      {selectionPos && selectedText && !locked && (
        <div
        ...
        >
          {isInlineProcessing ? (
            <div className="flex items-center space-x-2 py-1 px-3 text-indigo-600 font-semibold">
      ...
      {inlineFeedback && (
        <div className="w-full max-w-4xl mb-6 p-4 bg-indigo-50 border border-indigo-200 text-indigo-900 text-xs font-semibold rounded-xl flex items-center justify-between shadow-md animate-fadeIn">
      ...
      <div className="w-full max-w-4xl flex flex-col space-y-6">
        {locked && (
          <div className="p-3.5 bg-emerald-50 border border-emerald-300 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-sm">
            ...
            {onUnlockChapter && (
              <button
                ...
                className="shrink-0 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-black border border-black text-white text-xs font-bold hover:bg-neutral-800 disabled:opacity-50"
              >
                <Unlock size={14} />
                解锁重写
              </button>
        ...
        <div>
          <input
            ...
            readOnly={locked}
            placeholder="输入章节标题..."
            className={`w-full font-serif font-bold text-2xl lg:text-3xl text-black bg-white border-none focus:outline-none placeholder-neutral-400 tracking-tight pb-3 border-b border-neutral-200 ${
        ...
        <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-inner">
          <div className="text-xs font-bold text-indigo-700 mb-2 flex items-center justify-between">
            ...
            <span className="text-[11px] text-slate-500 font-mono">{chapter.summary.length} 字</span>
          </div>
          <textarea
            rows={2}
        ...
        <div className="rounded-xl border border-amber-200 bg-amber-50/50 shadow-sm overflow-hidden">
          <button
            ...
            className="w-full px-4 py-2.5 flex items-center justify-between text-xs font-bold text-amber-950 hover:bg-amber-50/80"
          >
            <span className="flex items-center gap-1.5">
              <StickyNote size={14} className="text-amber-700" />
              作者批注
              {chapter.authorNotes?.trim() ? (
                <span className="text-[10px] font-mono font-normal text-amber-800/80">
                  · {chapter.authorNotes.trim().length} 字
                </span>
              ) : (
                <span className="text-[10px] font-normal text-amber-800/60">· 改稿意图 / 读者反馈 / 待修点</span>
              )}
            </span>
          ...
          {notesOpen && (
            <div className="px-4 pb-3">
              <textarea
                ...
                placeholder="例：读者说节奏慢 → 下章压缩过渡；此处战力需与第3章呼应；待修：人名「玄清」是否统一…"
                disabled={isGenerating}
                className="w-full text-xs text-slate-800 bg-white border border-amber-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-300/50 resize-y min-h-[72px] placeholder-slate-400 leading-relaxed"
              />
              <p className="text-[10px] text-amber-900/70 mt-1.5">
                不进正文、导出 Markdown 会单独列出；锁定后仍可改批注。
              </p>
            </div>
          )}
        </div>
        ...
        <div className="relative min-h-[620px] flex-1 flex flex-col">
          <p className="text-[10px] text-slate-500 mb-2">
            划选 ≥4 字弹出局部精修：去AI味 / 压缩 / 扩写 / 更克制 / 更狠 / 截升华 / 硬伤瞥（自动带记忆与写前意图）。
            {wavyRanges.length > 0 && (
          ...
          <div className="relative flex-1 min-h-[620px]">
            ...
            {wavyRanges.length > 0 && (
              <div
                ref={proseMirrorRef}
                aria-hidden
                className="prose-mirror-layer absolute inset-0 z-0 w-full h-full font-serif text-base lg:text-lg text-black bg-white leading-loose overflow-hidden"
                style={{
                  ...proseFontStyle,
                  padding: 0,
                }}
            ...
            <textarea
              ...
              onMouseUp={locked ? undefined : captureSelection}
              onKeyUp={locked ? undefined : captureSelection}
              placeholder={
                isGenerating
                  ? '✍️ AI 引擎正在流式执笔中...'
                  : locked
              ...
              readOnly={locked}
              className={`relative z-10 w-full h-full min-h-[620px] font-serif text-base lg:text-lg border-none focus:outline-none resize-none leading-loose placeholder-neutral-400 selection:bg-rose-100 selection:text-black ${
                wavyRanges.length > 0
          ...
          <div className="py-4 border-t border-slate-200 text-right text-xs text-slate-500 flex justify-between items-center font-mono">
            <div className="flex items-center space-x-3">
              <span>
                状态: <strong className="text-indigo-700 font-medium">{chapter.status}</strong>
              </span>
