    ...
    <header className="bg-white border-b border-neutral-200 px-6 py-3 flex items-center justify-between sticky top-0 z-50 select-none text-black shadow-sm">
      ...
      <div className="flex items-center space-x-4 min-w-0">
        <button
          ...
          className="flex items-center space-x-2.5 bg-slate-100 hover:bg-slate-200 px-3.5 py-1.5 rounded-xl border border-slate-200 transition-all text-sm group shadow-sm shrink-0"
          title="切换小说项目或打开书库"
        >
          ...
          <span className="font-serif font-bold text-base text-black tracking-wide group-hover:text-indigo-700 transition-colors max-w-[12rem] truncate">
            {project.title || '无标题小说'}
          </span>
        ...
        <div className="hidden md:flex items-center space-x-2 shrink-0">
          <span className="text-xs text-indigo-700 bg-indigo-50 border border-indigo-200 px-2.5 py-0.5 rounded-full font-sans font-medium">
            {project.genre || '未设定分类'}
          </span>
          <button
            onClick={onOpenWizard}
            className="flex items-center space-x-1 text-xs text-white bg-black hover:bg-neutral-800 border border-black px-2.5 py-0.5 rounded-full transition-all font-medium"
          ...
          {onOpenReadingPreview && (
            <button
              ...
              className="flex items-center space-x-1 text-xs text-stone-700 hover:text-stone-900 bg-stone-100 hover:bg-stone-200 border border-stone-300 px-2.5 py-0.5 rounded-full transition-all font-medium"
              title="纯阅读预览正文"
            >
      ...
      {/* 中间：四大核心模块工作区导航 Tab */}
      <nav className="flex items-center space-x-1 bg-slate-100 p-1 rounded-xl border border-slate-200 shadow-inner shrink-0">
        <button
